function shoppingCartLocalStorage(responseText__JSON = '') {
  ...
  function countCart() {
    ...
    if (amountInCart > 0) {
      ...
      if (!nav__cartAndMember__Container1080__Amount__Check) {
        ...
        nav__cartAndMember__Container1080__Amount.className = 'nav__cartAndMember__Container1080__Amount';
        let nav__cartAndMember__Container1080__Amount__T = document.createTextNode(amountInCart);
        nav__cartAndMember__Container1080__Amount.appendChild(nav__cartAndMember__Container1080__Amount__T);
        nav__cartAndMember__Container1080__Div.appendChild(nav__cartAndMember__Container1080__Amount)
      } else {
        nav__cartAndMember__Container1080__Amount__Check.innerHTML = amountInCart
      }

      let cartAndMember__Container__Cart__Amount__Check = document.querySelector('.cartAndMember__Container__Cart__Amount');
      ...
      if (!cartAndMember__Container__Cart__Amount__Check) {
        ...
        let cartAndMember__Container__Cart__Img = document.querySelector('.cartAndMember__Container__Cart Img');

        let cartAndMember__Container__Cart__Amount = document.createElement('div');
        cartAndMember__Container__Cart__Amount.className = 'cartAndMember__Container__Cart__Amount';
        let nav__cartAndMember__Container1080__Amount__T2 = document.createTextNode(amountInCart);
        cartAndMember__Container__Cart__Amount.appendChild(nav__cartAndMember__Container1080__Amount__T2);
        cartAndMember__Container__Cart.insertBefore(cartAndMember__Container__Cart__Amount, cartAndMember__Container__Cart__Img)
      } else {
  ...
  let shoppingCartButton = document.querySelector('.productPage__Container__Sub__Info__Cart');
  ...
  function addToCartTest() {
    ...
    let productPage__Container__Sub__Info__Amount__Container__Num = document.querySelector('.productPage__Container__Sub__Info__Amount__Container__Num').innerHTML;
    let amount__V = Number(productPage__Container__Sub__Info__Amount__Container__Num)
    if (productPage__Container__Sub__Info__Color__block__Selected && productPage__Container__Sub__Info__Size__Size__Block__Selected && amount__V > 0) {
      addToCart();
    }
  }
  ...
  function addToCart() {
    ...
    let color = document.querySelector('.productPage__Container__Sub__Info__Color__block__Selected img').style.backgroundColor.replace(/\s/g, '').match(/^rgba?\((\d+),(\d+),(\d+)/i);
    let color__Value = `${Number(color[1]).toString(16)}${Number(color[2]).toString(16)}${Number(color[3]).toString(16)}`.toUpperCase();
    ...
    if (storageArray.length > 0) {
      ...
      storageArray.forEach(element => {
        if (element.id === inputObject.id && element.color === inputObject.color && element.size === inputObject.size) {
          element.amount = String(Number(element.amount) + Number(inputObject.amount));
          for (let va = 0; va < responseText__JSON.variants.length; va++) {
            if (responseText__JSON.variants[va]['color_code'] === inputObject.color && responseText__JSON.variants[va]['size'] === inputObject.size) {
              var maxAmount = responseText__JSON.variants[va]['stock'];
            }
          }

          element.amount = maxAmount > element.amount ? element.amount : String(maxAmount);
    ...
    let productPage__Container__Sub__Info__Cart = document.querySelector('.productPage__Container__Sub__Info__Cart');
    ...
    let productPage__Container__Sub__Info__Color__block__Selected = document.querySelector('.productPage__Container__Sub__Info__Color__block__Selected');
    productPage__Container__Sub__Info__Color__block__Selected.className = 'productPage__Container__Sub__Info__Color__block';

    let productPage__Container__Sub__Info__Size__Size__Block__Selected = document.querySelector('.productPage__Container__Sub__Info__Size__Size__Block__Selected');
    productPage__Container__Sub__Info__Size__Size__Block__Selected.className = 'productPage__Container__Sub__Info__Size__Size__Block';

    let productPage__Container__Sub__Info__Amount__Container__Num = document.querySelector('.productPage__Container__Sub__Info__Amount__Container__Num');

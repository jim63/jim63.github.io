...
switch (window.location.href.split('?')[1]) {
  ...
  default:
    if (window.location.href.split('?').length > 1) {
      productXMLRequest(`products/search?keyword=${decodeURI(window.location.href.split('?')[2].split('=')[1])}`, 0)
    } else {
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState === 4 && xhr.status === 200) {
      function callIAmNotAnonymous() {
        ...
        if (document.readyState === 'complete' && product__Container__Rect.bottom < 1000 && isLoad === false) {
...
const creatElement = (responseTextCreateElement) => {
  if (JSON.parse(responseTextCreateElement)["data"].length === 0) {
    ...
    for (let i = 0; i < JSON.parse(responseTextCreateElement).data.length; i++) {
      ...
      var product__Container__Each__Image__CreateElement__A = document.createElement('a');
      product__Container__Each__Image__CreateElement__A.href = `./product.html?id=${items.id}`;
      ...
      product__Container__Each__Image__CreateElement__A.appendChild(product__Container__Each__Image__CreateElement);
      ...
      product__Container__Each__CreateElement.appendChild(product__Container__Each__Image__CreateElement__A);

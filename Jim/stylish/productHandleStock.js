  function productHandleStock(responseText__JSON) {
    function checkStockSize() {
      ...
      if (productPage__Container__Sub__Info__Color__block__Selected) {
        ...
        for (let i = 0; i < responseText__JSON['variants'].length; i++) {
          if (responseText__JSON['variants'][i]['color_code'] === color__Value__Hex) {
            if (responseText__JSON['variants'][i]['stock'] === 0) {
              ...
              for (let j = 0; j < productPage__Container__Sub__Info__Size__Size__Block.length; j++) {
                if (productPage__Container__Sub__Info__Size__Size__Block[j].innerHTML === `<p>${responseText__JSON['variants'][i]['size']}</p>`) {
    ...
    function toggleOutOfStockSize() {
      ...
      outOfStock.forEach(element => {
        element.className = "productPage__Container__Sub__Info__Size__Size__Block";
      })
    }
    ...
    function plus() {
      ...
      if (productPage__Container__Sub__Info__Color__block__Selected && productPage__Container__Sub__Info__Size__Size__Block__Selected) {
        ...
        maxAmount = findMaxAmount(responseText__JSON, color__Value__Hex, size__Value);
    ...
    function findMaxAmount(responseText__JSON, color__Value__Hex, size__Value) {
      for (let i = 0; i < responseText__JSON['variants'].length; i++) {
        if (responseText__JSON['variants'][i]['color_code'] === color__Value__Hex && responseText__JSON['variants'][i]['size'] === size__Value) {
    ...
    document.addEventListener('click', () => {
      ...
      if (productPage__Container__Sub__Info__Color__block__Selected && productPage__Container__Sub__Info__Size__Size__Block__Selected) {
        var maxAmount = findMaxAmount(responseText__JSON, color__Value__Hex, size__Value);
        if (maxAmount === 0) {
